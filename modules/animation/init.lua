...
local function new()
  ...
  GLib.timeout_add(GLib.PRIORITY_DEFAULT, ANIMATION_FRAME_DELAY, function()
    for index, ani in ipairs(ret._private.animations) do
      if ani.state == true then
        -- compute delta time
        local time = GLib.get_monotonic_time()
        local delta = time - ani.last_elapsed
        ani.last_elapsed = time

        -- If pos is true, the ani has ended
        local pos = ani.tween:update(delta)
        if pos == true then
          -- Loop the ani, don't end it.
          -- Useful for widgets like the spinning cicle
          if ani.loop == true then
            ani.tween:reset()
          else
            -- Snap to end
            ani.pos = ani.tween.target
            ani:fire(ani.pos)
            ani:emit_signal("update", ani.pos)

            ani.state = false
            ani.ended:fire(pos)
            table.remove(ret._private.animations, index)
            ani:emit_signal("ended", ani.pos)
          end
          -- ani in process, keep updating
        else
          ani.pos = pos
          ani:fire(ani.pos)
          ani:emit_signal("update", ani.pos)
